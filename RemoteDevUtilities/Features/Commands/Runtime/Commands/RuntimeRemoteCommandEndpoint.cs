...
namespace HP.Utilities.RemoteDevUtilities.Commands
{
    ...
    internal sealed class RuntimeRemoteCommandEndpoint : IRuntimeRemoteEndpoint, IRuntimeRemoteSessionListener
    {
        ...
        public void Handle(RemoteEnvelope envelope)
        {
            ...
            {
                case RemoteCommandMessageTypes.CatalogRequest:
                    SendCatalog(envelope.RequestId);
                    break;
                case RemoteCommandMessageTypes.ExecuteRequest:
                    Execute(envelope);
                    break;
            }
        }
        ...
        private void SendCatalog(long requestId)
        {
            ...
            if (behaviour == null)
            {
                _context.Sender.Send(RemoteCommandMessageTypes.CatalogResponse, requestId, new RemoteCommandCatalogResponse
                {
        ...
        {
            _context.Sender.Send(RemoteCommandMessageTypes.ExecuteResponse, requestId, new RemoteCommandExecuteResponse
            {
                Success = success,
