...
namespace SAS.Utilities.RemoteDevUtilities.Editor.FrameRecorder
{
    internal sealed class RemoteFrameRecorderPanel : IRemoteSceneInspectorMode
    {
        ...
        private static readonly string[] InspectorScopeLabels = { "Selected Object", "Hierarchy Only", "All Objects" };
        ...
        public bool IsAvailable => RemoteDevUtilitiesProjectSettings.instance.Runtime.EnableExperimentalFrameRecorder;
        ...
        public bool Draw(RemoteDevUtilitiesClient client, bool connected, Rect windowRect)
        {
            ...
            RemoteRuntimeSceneInspectorClient liveInspector = client.GetRequiredFeature<RemoteRuntimeSceneInspectorClient>();
            ...
            {
                EditorGUILayout.HelpBox("Offline replay. The recorded hierarchy and Inspector remain fully available without a running Player.", MessageType.Info);
            }
            else
            {
                EditorGUILayout.HelpBox("Connect to a development Player to record frames, or open a saved recording from disk.", MessageType.Info);
            }
        ...
        private void DrawToolbar(RemoteFrameRecorderClient client, RemoteRuntimeSceneInspectorClient liveInspector)
        {
            ...
            bool missingSelection = scope == RemoteFrameRecorderInspectorScope.SelectedObject && inspectedObjectId <= 0;

            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            using (new EditorGUI.DisabledScope(busy || state == RemoteFrameRecorderState.Recording || missingSelection))
            {
                if (GUILayout.Button("Start Recording", EditorStyles.toolbarButton, GUILayout.Width(100f)))
                {
                    ResetReplayView();
                    client.Start(_capacity, WidthValues[_widthIndex], _jpegQuality, scope, inspectedObjectId);
                }
            }

            using (new EditorGUI.DisabledScope(busy || (state != RemoteFrameRecorderState.Recording && state != RemoteFrameRecorderState.Sealed)))
            {
                string fetchLabel = state == RemoteFrameRecorderState.Sealed ? "Download Frames" : "Stop & Download";
            ...
            {
                if (GUILayout.Button("Release Player Memory", EditorStyles.toolbarButton, GUILayout.Width(136f)))
                {
                    _playing = false;
                    client.Release();
                }
            ...
            _capacity = Mathf.Clamp(EditorGUILayout.DelayedIntField(_capacity, GUILayout.Width(48f)), RemoteFrameRecorderLimits.MinimumCapacity, RemoteFrameRecorderLimits.MaximumCapacity);
            GUILayout.Label("Image Width", EditorStyles.miniLabel, GUILayout.Width(66f));
            _widthIndex = EditorGUILayout.Popup(_widthIndex, WidthLabels, EditorStyles.toolbarPopup, GUILayout.Width(48f));
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            GUILayout.Label("Recorded Data", GUILayout.Width(86f));
            _inspectorScopeIndex = EditorGUILayout.Popup(_inspectorScopeIndex, InspectorScopeLabels, GUILayout.Width(140f));
            if (scope == RemoteFrameRecorderInspectorScope.SelectedObject)
            {
                string selectedName = liveInspector.Inspection?.Details?.Name;
                GUILayout.Label(inspectedObjectId > 0 ? $"Selected: {selectedName ?? inspectedObjectId.ToString()}" : "Select an object in the Hierarchy below", EditorStyles.miniLabel, GUILayout.MinWidth(180f));
            }

            _freezeWhenFetched = EditorGUILayout.ToggleLeft("Pause Player after stopping", _freezeWhenFetched, GUILayout.Width(190f));
            ...
            {
                EditorGUILayout.HelpBox("All Objects uses cached metadata, curated Unity component readers, and granular snapshot reuse. Unity values must still be read on the main thread, so large or shader-heavy scenes can remain expensive.", MessageType.Warning);
            }

            if (_capacity > 100)
            {
                EditorGUILayout.HelpBox("Large recordings use more Player memory and take longer to download. Use a smaller image width and Selected Object or Hierarchy Only when approaching 300 frames.", MessageType.Info);
            }
        }
        ...
        {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            if (GUILayout.Button("Open Recording...", EditorStyles.toolbarButton, GUILayout.Width(112f)))
            {
                string path = EditorUtility.OpenFilePanel("Open Frame Recording", string.Empty, RemoteFrameRecordingStore.FileExtension);
            ...
            {
                if (GUILayout.Button("Save Recording...", EditorStyles.toolbarButton, GUILayout.Width(112f)))
                {
                    long recordingId = client.Manifest?.RecordingId ?? 0;
                    string defaultName = recordingId == 0 ? "Frame Recording" : $"Frame Recording {recordingId}";
                    string directory = string.IsNullOrEmpty(client.RecordingFilePath) ? string.Empty : System.IO.Path.GetDirectoryName(client.RecordingFilePath);
                    string path = EditorUtility.SaveFilePanel("Save Frame Recording", directory, defaultName, RemoteFrameRecordingStore.FileExtension);
            ...
            if (!string.IsNullOrEmpty(client.RecordingFilePath))
                GUILayout.Label("Local: " + System.IO.Path.GetFileName(client.RecordingFilePath), EditorStyles.miniLabel);
            EditorGUILayout.EndHorizontal();
        }
        ...
        private static void DrawStatus(RemoteFrameRecorderClient client)
        {
            ...
            if (client.IsDownloading)
            {
                ...
                float frameProgress = client.CurrentFrameDownloadProgress;
                float overallProgress = (client.DownloadedFrameCount + frameProgress) / total;
                string chunkProgress = frameProgress > 0f ? $" ({frameProgress:P0})" : string.Empty;
                Rect progress = EditorGUILayout.GetControlRect(false, 18f);
                EditorGUI.ProgressBar(progress, overallProgress, $"Downloading frame {Mathf.Min(client.DownloadedFrameCount + 1, total)} of {total}{chunkProgress}");
                return;
            }
            ...
            switch (status.State)
            {
                case RemoteFrameRecorderState.Recording:
                    ...
                    EditorGUILayout.HelpBox($"Recording each rendered frame locally ({inspectorScope}). The Player retains its latest {status.Capacity} frames and sends no frame data until requested.", MessageType.Info);
                    break;
                case RemoteFrameRecorderState.Finalizing:
                    EditorGUILayout.HelpBox($"Finalizing {status.PendingFrameCount} background frame operation(s)...", MessageType.Info);
                    break;
                case RemoteFrameRecorderState.Sealed:
                    string gaps = status.MissedFrameCount == 0 ? "continuous" : $"{status.MissedFrameCount} missed";
                    string readback = status.UsesAsyncGpuReadback ? "async GPU readback" : "synchronous readback fallback";
                    string graphSavings = status.SceneGraphBytesSaved > 0 ? $", graph reuse saved {FormatBytes(status.SceneGraphBytesSaved)}" : string.Empty;
                    EditorGUILayout.LabelField($"Stopped: {status.CapturedFrameCount}/{status.Capacity} frames, {gaps}, {FormatBytes(status.StoredBytes)}{graphSavings}, {readback} + background encoding", EditorStyles.centeredGreyMiniLabel);
                    if (!string.IsNullOrEmpty(status.Warning))
                        EditorGUILayout.HelpBox(status.Warning, MessageType.Warning);
                    break;
                default:
                    EditorGUILayout.HelpBox("Start recording, reproduce the problem, then stop and download the latest consecutive frames.", MessageType.None);
                    break;
            }
        }

        private void DrawReplay(RemoteFrameRecorderClient client, RemoteRuntimeSceneInspectorClient liveInspector, Rect windowRect, bool connected)
        {
            ...
            if (frames == null || frames.Count == 0)
            {
                ...
                if (connected)
                    DrawLiveSelection(liveInspector, client.Status?.State ?? RemoteFrameRecorderState.Idle, windowRect);
                return;
            }
        ...
        private void DrawLiveSelection(RemoteRuntimeSceneInspectorClient liveInspector, RemoteFrameRecorderState recorderState, Rect windowRect)
        {
            ...
            }, (_, _) =>
            {
                ...
                {
                    EditorGUILayout.HelpBox("Select a GameObject from the Hierarchy. Selected Object recording stores that object's complete historical Inspector data on every frame.", MessageType.Info);
                }
                else if (recorderState == RemoteFrameRecorderState.Recording)
                {
                    EditorGUILayout.HelpBox("Recording is active. The object chosen when Start Recording was pressed is being recorded; changing the current selection does not change the active recording.", MessageType.Info);
                }
                else
                {
                    EditorGUILayout.HelpBox($"Selected: {liveInspector.Inspection?.Details?.Name ?? liveInspector.InspectionObjectId.ToString()}. Start recording, reproduce the issue, then select Stop & Download.", MessageType.None);
                }
        ...
        {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            if (GUILayout.Button(_playing ? "Pause" : "Play", EditorStyles.toolbarButton, GUILayout.Width(46f)))
            {
                _playing = !_playing;
                if (_playing)
            ...
            GUILayout.Label($"{_selectedFrameIndex + 1}/{frames.Count}  Unity frame {info.UnityFrame}", EditorStyles.miniLabel, GUILayout.Width(150f));
            GUILayout.Label("Speed", EditorStyles.miniLabel, GUILayout.Width(34f));
            int speedIndex = EditorGUILayout.Popup(_playbackSpeedIndex, PlaybackSpeedLabels, EditorStyles.toolbarPopup, GUILayout.Width(52f));
        ...
        private void DrawImage(RemoteFrameReplayFrame frame, float width)
        {
            ...
            else
            {
                ...
                float imageHeight = Mathf.Clamp(imageWidth * frame.Info.Height / Mathf.Max(1f, frame.Info.Width), 180f, 620f);
            ...
            EditorGUILayout.LabelField($"Frame {frame.Info.UnityFrame}  |  {frame.Info.Width}x{frame.Info.Height}  |  t={frame.Info.RealtimeSeconds:F3}s", EditorStyles.centeredGreyMiniLabel);
            EditorGUILayout.LabelField($"Image {FormatBytes(frame.Info.ImageBytes)}  |  Inspector {FormatBytes(frame.Info.SceneGraphBytes)}", EditorStyles.centeredGreyMiniLabel);
        }
        ...
        private void TickPlayback()
        {
            ...
            _nextPlaybackAt = now + CalculatePlaybackDelay(frames, _selectedFrameIndex, PlaybackSpeedValues[_playbackSpeedIndex]);
            _repaint?.Invoke();
        }

        private void ScheduleNextPlaybackFrame(IReadOnlyList<RemoteFrameReplayFrame> frames)
        {
            _nextPlaybackAt = EditorApplication.timeSinceStartup + CalculatePlaybackDelay(frames, _selectedFrameIndex, PlaybackSpeedValues[_playbackSpeedIndex]);
        }

        internal static double CalculatePlaybackDelay(IReadOnlyList<RemoteFrameReplayFrame> frames, int currentIndex, float playbackSpeed)
        {
            ...
            {
                interval = frames[index + 1].Info.RealtimeSeconds - frames[index].Info.RealtimeSeconds;
            }
            else
            {
                interval = frames[index].Info.RealtimeSeconds - frames[index - 1].Info.RealtimeSeconds;
            }
