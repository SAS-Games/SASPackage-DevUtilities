...
namespace SAS.Utilities.RemoteDevUtilities.MiniTools
{
    ...
    internal sealed class RuntimeRemoteMiniToolEndpoint : IRuntimeRemoteEndpoint, IRuntimeRemoteSessionListener
    {
        ...
        {
            RemoteMiniToolMessageTypes.CatalogRequest,
            RemoteMiniToolMessageTypes.SubscriptionRequest,
            RemoteMiniToolMessageTypes.ActionRequest
        };
        ...
        public void Handle(RemoteEnvelope envelope)
        {
            ...
            {
                case RemoteMiniToolMessageTypes.CatalogRequest:
                    SendCatalog(envelope.RequestId);
                    break;
                case RemoteMiniToolMessageTypes.SubscriptionRequest:
                    SetSubscription(envelope);
                    break;
                case RemoteMiniToolMessageTypes.ActionRequest:
        ...
        public void Tick()
        {
            ...
            foreach (ProviderState state in _providers.Values)
            {
                ...
                if (RequiresSample(state.DataChannels) && now >= state.NextSampleTime)
                {
                    ...
                    if (sample != null)
                        _context.Sender.Send(RemoteMiniToolMessageTypes.Sample, 0, sample);
                }
                ...
                if (batch != null)
                    _context.Sender.Send(RemoteMiniToolMessageTypes.StreamBatch, 0, batch);
            }
        }
        ...
        private void SendCatalog(long requestId)
        {
            ...
            _context.Sender.Send(RemoteMiniToolMessageTypes.CatalogResponse, requestId, new RemoteMiniToolCatalogResponse { Tools = descriptors.ToArray() });
        }
        ...
        {
            _context.Sender.Send(RemoteMiniToolMessageTypes.SubscriptionResponse, requestId, new RemoteMiniToolSubscriptionResponse
            {
                ToolId = toolId,
                Success = success,
        ...
        {
            _context.Sender.Send(RemoteMiniToolMessageTypes.ActionResponse, requestId, new RemoteMiniToolActionResponse
            {
                ToolId = toolId,
                ActionId = actionId,
                Success = success,
