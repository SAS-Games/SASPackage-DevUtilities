...
namespace SAS.Utilities.RemoteDevUtilities.Editor.UI.Panels
{
    ...
    internal sealed class RemoteMiniToolsPanel : IRemoteWorkspacePanel
    {
        ...
        {
            _repaint = repaint;
            RemoteMiniToolVisibilitySettings.Changed += OnSettingsChanged;
        }

        bool IRemoteWorkspacePanel.Draw(RemoteDevUtilitiesClient client, bool connected, Rect windowRect)
        {
            Draw(client.GetRequiredFeature<RemoteMiniToolClient>(), connected);
            return false;
        }
        ...
        private void OnSettingsChanged() => _repaint?.Invoke();
