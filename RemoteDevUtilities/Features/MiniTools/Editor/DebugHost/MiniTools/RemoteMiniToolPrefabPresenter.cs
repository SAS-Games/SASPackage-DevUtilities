...
namespace SAS.Utilities.RemoteDevUtilities.Editor.DebugHost.MiniTools
{
    internal sealed class RemoteMiniToolPrefabPresenter : IDisposable
    {
        ...
        private readonly RemoteMiniToolClient _miniTools;
        ...
        {
            _client = client;
            _miniTools = client.GetRequiredFeature<RemoteMiniToolClient>();
        ...
        private void Refresh()
        {
            ...
            for (int i = 0; i < activeDefinitions.Length; i++)
            {
                ...
                bool subscribed = _miniTools.IsSubscriptionRequested(definition.ToolId, RemoteMiniToolSubscriptionOwner.DebugHost);
                if (!subscribed)
                {
                ...
                {
                    view = new RemoteMiniToolPrefabView(definition, i, actionId => _miniTools.ExecuteAction(definition.ToolId, actionId));
                    if (!view.IsValid)
                    {
                ...
                if (_miniTools.Samples.TryGetValue(definition.ToolId, out RemoteMiniToolSample sample) && (!_presentedSamples.TryGetValue(definition.ToolId, out RemoteMiniToolSample presentedSample) || !ReferenceEquals(presentedSample, sample)))
                {
                    _miniTools.TryGetTool(definition.ToolId, out RemoteMiniToolDescriptor descriptor);
                ...
                _miniTools.DrainStreamBatches(definition.ToolId, _pendingStreamBatches);
        ...
        private void RemoveInactiveViews(IReadOnlyList<RemoteMiniToolPrefabDefinition> definitions)
        {
            ...
            {
                if (!activeToolIds.Contains(entry.Key) || !_miniTools.IsSubscriptionRequested(entry.Key, RemoteMiniToolSubscriptionOwner.DebugHost))
        ...
        private RemoteMiniToolPrefabDefinition[] IncludeTargetTools(IReadOnlyList<RemoteMiniToolPrefabDefinition> definitions)
        {
            ...
            foreach (RemoteMiniToolDescriptor descriptor in _miniTools.Tools ?? Array.Empty<RemoteMiniToolDescriptor>())
            {
