...
namespace HP.Utilities.RemoteDevUtilities.Editor.MiniTools.Configuration
{
    internal sealed class RemoteMiniToolConfigurationWindow : EditorWindow
    {
        ...
        private RemoteMiniToolClient _miniTools;
        ...
        private void OnGUI()
        {
            ...
            if (_miniTools != null)
                settings.RegisterCatalog(_miniTools.Tools);
        ...
        private void BuildToolList(RemoteMiniToolVisibilitySettings settings)
        {
            ...
            foreach (RemoteMiniToolDescriptor descriptor in settings.KnownTools)
            {
        ...
        private void UnsubscribeHiddenTools(RemoteMiniToolVisibilitySettings settings)
        {
            if (_client == null)
                return;
            foreach (RemoteMiniToolDescriptor descriptor in _miniTools.Tools)
            {
                if (descriptor == null || string.IsNullOrWhiteSpace(descriptor.Id))
                    continue;
                if (!settings.IsVisible(descriptor.Id))
        ...
        {
            _miniTools?.ClearSubscriptions(toolId);
        }
        ...
        private void TryAttachClient()
        {
            ...
            try
            {
                ...
                _miniTools = _client.GetRequiredFeature<RemoteMiniToolClient>();
                _client.StateChanged += Repaint;
