...
namespace SAS.Utilities.RemoteDevUtilities.Editor.RuntimeSceneInspector
{
    internal sealed class RemoteRuntimeSceneInspectorClient : IRemoteEditorFeatureClient
    {
        ...
        public RemoteSceneInspectorHierarchyResponse Hierarchy { get; private set; } =
            new();
        ...
        public void RequestHierarchy(bool forceRefresh)
        {
            _session.Send(
                ...
                new RemoteSceneInspectorHierarchyRequest { ForceRefresh = forceRefresh });
        }
        ...
        public void Inspect(long objectId)
        {
            ...
            _inspectionRequestId = _session.Send(
                ...
                new RemoteSceneInspectorInspectRequest { ObjectId = objectId });
        }
            ...
            switch (envelope.MessageType)
            {
                case RemoteMessageTypes.SceneInspectorHierarchyResponse:
                    if (RemoteProtocolSerializer.TryDeserializePayload(
                            envelope,
                            out RemoteSceneInspectorHierarchyResponse hierarchy,
                            out _))
                        Hierarchy = hierarchy;
                    break;
                case RemoteMessageTypes.SceneInspectorInspectResponse:
                    ...
                    if (RemoteProtocolSerializer.TryDeserializePayload(
                            envelope,
                            out RemoteSceneInspectorInspectResponse inspection,
                            out _))
                ...
                case RemoteMessageTypes.SceneInspectorCommandResponse:
                    if (RemoteProtocolSerializer.TryDeserializePayload(
                            envelope,
                            out RemoteSceneInspectorCommandResponse commandResult,
                            out _))
                    {
