...
namespace SAS.Utilities.RemoteDevUtilities.Editor.RuntimeDebugger
{
    internal sealed class RemoteMaterialInspectorView
    {
            ...
            int scope)
        {
            ...
            {
                Kind = RemoteDebuggerCommandKind.RestoreMaterial,
            ...
            string key)
        {
            ...
            using (new EditorGUI.DisabledScope(value == (property.Value ?? string.Empty)))
            {
                if (GUILayout.Button("Apply", GUILayout.Width(48f)))
                {
                    client.Execute(new RemoteDebuggerCommandRequest
                    {
                        Kind = RemoteDebuggerCommandKind.SetShaderProperty,
                        RendererId = rendererId,
                        MaterialIndex = slot.MaterialIndex,
        ...
        private static void DrawResetButton(
            ...
            int scope)
        {
            using (new EditorGUI.DisabledScope(!property.HasDebuggerOverride))
            {
                ...
                client.Execute(new RemoteDebuggerCommandRequest
                {
                    Kind = RemoteDebuggerCommandKind.RestoreShaderProperty,
                    RendererId = rendererId,
                    MaterialIndex = slot.MaterialIndex,
                    PropertyId = property.PropertyId,
                    MaterialScope = scope
