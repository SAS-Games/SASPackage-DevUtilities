...
namespace SAS.Utilities.RemoteDevUtilities.Editor.MiniTools.Configuration
{
    internal sealed class RemoteMiniToolConfigurationWindow : EditorWindow
    {
        ...
        private readonly Dictionary<string, string> _presentationErrors =
            new(StringComparer.OrdinalIgnoreCase);
        private readonly Dictionary<string, string> _commandErrors =
            new(StringComparer.OrdinalIgnoreCase);
        private readonly Dictionary<string, bool> _overrideFoldouts =
            new(StringComparer.OrdinalIgnoreCase);
        ...
        private void OnGUI()
        {
            ...
            RemoteMiniToolPresentationSettings presentationSettings =
                RemoteMiniToolPresentationSettings.instance;
            RemoteMiniToolCommandSettings commandSettings =
            ...
            settings.RegisterCatalog(
                RemoteMiniToolEditorDiscovery.Descriptors);
        ...
        private void DrawHeader(RemoteMiniToolVisibilitySettings settings)
        {
            ...
            EditorGUILayout.LabelField(
                "Every installed definition automatically feeds the Player, Native Workspace, Debug Host, and command routing. " +
                "This window controls visibility and optional project overrides.",
            ...
            if (!string.IsNullOrWhiteSpace(_initializationError))
                ...
                EditorGUILayout.HelpBox(
                    "Installed definitions are available without a Player connection. Connect only to verify a specific build or discover a legacy runtime-only provider.",
                    MessageType.Info);
        ...
        private void DrawSearch()
        {
            ...
            if (!string.IsNullOrEmpty(_search) &&
                GUILayout.Button("Clear", EditorStyles.toolbarButton, GUILayout.Width(45f)))
                _search = string.Empty;
            EditorGUILayout.EndHorizontal();
        }

        private void DrawToolList(
            RemoteMiniToolVisibilitySettings settings,
            RemoteMiniToolPresentationSettings presentationSettings,
            RemoteMiniToolCommandSettings commandSettings)
        {
            ...
            RemoteMiniToolDescriptor descriptor)
        {
            ...
            bool forget =
                ...
                GUILayout.Button(
                    new GUIContent(
                        "Forget",
                        "Remove this unavailable mini-tool and its project overrides from the remembered catalog."),
                    ...
                    out MiniToolRegistration registration))
            {
                ...
                bool expanded = _overrideFoldouts.TryGetValue(
                                    descriptor.Id,
                                    out bool stored) &&
                                stored;
            ...
            {
                EditorGUILayout.HelpBox(
                    "Legacy provider: create a Mini Tool Definition to use the unified registration pipeline.",
                    MessageType.Warning);
                DrawHostPrefab(presentationSettings, descriptor.Id);
                DrawCommandRouting(commandSettings, descriptor);
            }
        ...
        private static void DrawDefinition(
            MiniToolRegistration registration)
        {
            ...
            using (new EditorGUI.DisabledScope(true))
            {
                EditorGUILayout.ObjectField(
                    ...
                    typeof(MiniToolDefinition),
                    false);
            }

            if (GUILayout.Button(
                    registration.IsProjectOwned ? "Edit" : "View",
                    EditorStyles.miniButton,
                    GUILayout.Width(45f)))
            {
            ...
            string commandName =
                registration.Descriptor.Command?.Name;
            GameObject prefab = registration.LoadDebugHostPrefab();
            RemoteMiniToolCapabilities capabilities =
                registration.Descriptor.Capabilities;
            string nativeWorkspace =
                (capabilities &
                 RemoteMiniToolCapabilities.NativeWorkspaceFields) != 0
                    ? "Fields"
                    : "Not used";
            string debugHost =
                (capabilities &
                 RemoteMiniToolCapabilities.TypedDebugHostSnapshot) != 0
                    ? prefab == null
                        ? "Typed snapshot (generic fallback)"
                        : "Typed snapshot"
                    : prefab == null
                        ? "Generic fields"
                        : prefab.name;
            if ((capabilities &
                 RemoteMiniToolCapabilities.EventStream) != 0)
            {
            ...
            string summary =
                ...
                $"Command: {(string.IsNullOrWhiteSpace(commandName) ? "None" : commandName)}";
            EditorGUILayout.LabelField(
                summary,
                EditorStyles.centeredGreyMiniLabel);
        }

        private void DrawHostPrefab(
            RemoteMiniToolPresentationSettings settings,
            string toolId)
        {
            bool hasOverride = settings.TryGetOverride(
                toolId,
                out GameObject overridePrefab);
            bool hasDefault = MiniToolRegistry.TryGetDebugHostPrefab(
                toolId,
                out GameObject defaultPrefab);
            GameObject effectivePrefab =
                hasOverride && overridePrefab != null
                    ? overridePrefab
                    : defaultPrefab;
            ...
            GameObject selected = (GameObject)EditorGUILayout.ObjectField(
                new GUIContent(
                    "Host Prefab",
                    "Optional prefab instantiated in the Play Mode Debug Host."),
                effectivePrefab,
                typeof(GameObject),
                false);
            if (EditorGUI.EndChangeCheck())
            ...
            string source = hasOverride
                ? overridePrefab != null
                    ? "Project override"
                    : "Project override is missing"
                : hasDefault
                    ? "Package default"
                    : "No Host prefab assigned";
        ...
        private void DrawCommandRouting(
            RemoteMiniToolCommandSettings settings,
            RemoteMiniToolDescriptor descriptor)
        {
            ...
            bool hasMiniToolDefault =
                RemoteMiniToolCommandManifestResolver.TryCreateBinding(
                    descriptor,
                    out RemoteCommandPresentationBinding miniToolBinding);
            bool hasPackageDefault =
                RemoteCommandPresentationRegistry.TryGetDefinitionDefaultForMiniTool(
                    toolId,
                    out RemoteCommandPresentationBinding packageBinding);
            RemoteCommandPresentationBinding defaultBinding =
                hasMiniToolDefault
                    ? miniToolBinding
                    : hasPackageDefault
                        ? packageBinding
            ...
            string commandName = hasOverride
                ? commandOverride.CommandName
                : defaultBinding != null
                    ? defaultBinding.CommandName
                    : string.Empty;
            RemoteCommandRouting routing = hasOverride
                ? commandOverride.Routing
                : defaultBinding != null
                    ? defaultBinding.Routing
            ...
            EditorGUILayout.LabelField(
                "Optional command that starts or stops this Editor mini-tool. Leave it empty to disable command control.",
                EditorStyles.wordWrappedMiniLabel);

            EditorGUILayout.BeginHorizontal();
            EditorGUI.BeginChangeCheck();
            string selectedCommand = EditorGUILayout.DelayedTextField(
                new GUIContent(
                    "Command Name",
                    "Existing console command name, without its prefix or On/Off argument."),
                commandName);
            if (EditorGUI.EndChangeCheck())
            {
            ...
            {
                EditorGUI.BeginChangeCheck();
                RemoteCommandRouting selectedRouting =
                    (RemoteCommandRouting)EditorGUILayout.EnumPopup(
                        new GUIContent(
                            "When Command Runs",
                            "Choose whether the command executes in the build, controls the Editor tool, or does both."),
                        routing);
                if (EditorGUI.EndChangeCheck())
                {
                    ApplyCommandConfiguration(
            ...
            string source = hasOverride
                ? string.IsNullOrWhiteSpace(commandOverride.CommandName)
                    ? "Project override: command disabled"
                    : "Project override"
                : hasMiniToolDefault
                    ? "Mini-tool default"
                    : hasPackageDefault
        ...
        private void ApplyCommandConfiguration(
            RemoteMiniToolCommandSettings settings,
            string toolId,
            string commandName,
            ...
            RemoteCommandPresentationBinding defaultBinding)
        {
            ...
            if (defaultBinding != null &&
                ...
                routing == defaultBinding.Routing)
            {
                settings.ClearOverride(toolId);
                _commandErrors.Remove(toolId);
                return;
            ...
            foreach (RemoteCommandPresentationBinding registration in
                     RemoteCommandPresentationRegistry.GetRegistrations())
            {
                if (string.IsNullOrEmpty(normalizedCommandName) ||
                    !string.Equals(
                    ...
                    string.Equals(
                        registration.MiniToolId,
                        toolId,
                        StringComparison.OrdinalIgnoreCase))
                    continue;

                _commandErrors[toolId] =
                    $"Command '{normalizedCommandName}' is already assigned to mini-tool " +
                    $"'{registration.MiniToolId}'.";
                return;
            }
            ...
            foreach (RemoteMiniToolDescriptor candidate in _tools)
            {
                if (string.IsNullOrEmpty(normalizedCommandName) ||
                    string.Equals(
                        ...
                        toolId,
                        StringComparison.OrdinalIgnoreCase) ||
                    RemoteMiniToolCommandSettings.instance.TryGetOverride(
                        candidate?.Id,
                        out _) ||
                    !RemoteMiniToolCommandManifestResolver.TryCreateBinding(
                        candidate,
                        out RemoteCommandPresentationBinding
                            manifestBinding) ||
                    !string.Equals(
                        manifestBinding.CommandName,
                        normalizedCommandName,
                        StringComparison.OrdinalIgnoreCase))
                    continue;

                _commandErrors[toolId] =
                    $"Command '{normalizedCommandName}' is already recommended by mini-tool " +
                    $"'{manifestBinding.MiniToolId}'.";
                return;
            }

            if (!settings.SetOverride(
                    toolId,
                    normalizedCommandName,
                    routing,
                    out string error))
        ...
        private bool MatchesSearch(RemoteMiniToolDescriptor descriptor)
        {
            if (string.IsNullOrWhiteSpace(_search))
                return true;
            if (Contains(descriptor.Id, _search) ||
                Contains(descriptor.DisplayName, _search) ||
                Contains(descriptor.Description, _search))
                return true;

            if (RemoteMiniToolCommandSettings.instance.TryGetOverride(
                    descriptor.Id,
                    out RemoteMiniToolCommandOverride commandOverride))
            ...
            return RemoteCommandPresentationRegistry.TryGetDefinitionDefaultForMiniTool(
                       descriptor.Id,
                       out RemoteCommandPresentationBinding defaultBinding) &&
                   Contains(defaultBinding.CommandName, _search);
        }

        private void AddRememberedTool(
            string toolId,
            ISet<string> rememberedToolIds)
        {
            if (string.IsNullOrWhiteSpace(toolId) ||
                !rememberedToolIds.Add(toolId))
                return;
            ...
            _tools.Add(
                new RemoteMiniToolDescriptor
                {
                    ...
                    DisplayName = toolId,
                    Description =
                        "Connect to a Player to load this mini-tool's runtime description."
                });
        }
