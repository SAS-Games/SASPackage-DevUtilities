...
namespace SAS.Utilities.RemoteDevUtilities.Editor.DebugHost.MiniTools
{
    ...
    internal static class RemoteMiniToolStreamViewFactory
    {
        ...
        internal static IRemoteMiniToolStreamView[] Find(
            GameObject instance)
        {
                     ...
                     instance.GetComponentsInChildren<MonoBehaviour>(true))
            {
                         ...
                         behaviour.GetType().GetInterfaces())
                {
                    ...
                    Type adapterType =
                        typeof(RemoteMiniToolStreamView<>)
                            .MakeGenericType(eventType);
                    if (Activator.CreateInstance(
                            adapterType,
                            new object[] { behaviour }) is
                        IRemoteMiniToolStreamView view)
                    {
                        views.Add(view);
                    }
                }
    ...
    internal sealed class RemoteMiniToolStreamView<TEvent> :
        IRemoteMiniToolStreamView
        where TEvent : IMiniToolStreamEvent
    {
        ...
        public RemoteMiniToolStreamView(
            IMiniToolStreamView<TEvent> view)
        {
            _view = view ??
        ...
        {
            if (batch == null ||
                string.IsNullOrWhiteSpace(batch.EventTypeName) ||
                string.IsNullOrWhiteSpace(batch.EventsJson) ||
                !MatchesEventType(batch.EventTypeName))
            {
                return false;
            }

            RemoteMiniToolStreamPayload<TEvent> payload =
                JsonUtility.FromJson<
                    RemoteMiniToolStreamPayload<TEvent>>(
                    batch.EventsJson);
            _view.ApplyEvents(
                payload?.Events ?? Array.Empty<TEvent>(),
                batch.DroppedEventCount);
            return true;
        }

        private bool MatchesEventType(
