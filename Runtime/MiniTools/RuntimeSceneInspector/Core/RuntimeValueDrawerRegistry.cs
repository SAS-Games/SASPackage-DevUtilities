...
namespace SAS.Utilities.RuntimeSceneInspector.Core
{
    ...
    internal sealed class RuntimeStructDrawer : IRuntimeValueDrawer
    {
        ...
        public string Format(object value, Type t)
        {
            if (value is Vector2 v2)
                return Join(v2.x, v2.y);
            if (value is Vector2Int v2i)
                return Join(v2i.x, v2i.y);
            if (value is Vector3 v3)
                return Join(v3.x, v3.y, v3.z);
            if (value is Vector3Int v3i)
                return Join(v3i.x, v3i.y, v3i.z);
            ...
            if (value is Color c)
                return Join(c.r, c.g, c.b, c.a);
            if (value is Color32 c32)
                return Join(c32.r, c32.g, c32.b, c32.a);
            if (value is Rect r)
                return Join(r.x, r.y, r.width, r.height);
            if (value is Bounds b)
                return Join(b.center.x, b.center.y, b.center.z, b.size.x, b.size.y, b.size.z);
        ...
        public bool TryParse(string text, Type t, out object value, out string error)
        {
            try
            {
                ...
                else if (t == typeof(Vector2Int) && f.Length == 2)
                    value = new Vector2Int((int)f[0], (int)f[1]);
                else if (t == typeof(Vector3) && f.Length == 3)
                    value = new Vector3(f[0], f[1], f[2]);
                else if (t == typeof(Vector3Int) && f.Length == 3)
                    value = new Vector3Int((int)f[0], (int)f[1], (int)f[2]);
                else if (t == typeof(Vector4) && f.Length == 4)
                    value = new Vector4(f[0], f[1], f[2], f[3]);
                else if (t == typeof(Quaternion) && f.Length == 3)
                    value = Quaternion.Euler(f[0], f[1], f[2]);
                else if (t == typeof(Color) && f.Length == 4)
                    value = new Color(f[0], f[1], f[2], f[3]);
                else if (t == typeof(Color32) && f.Length == 4)
                    value = new Color32((byte)f[0], (byte)f[1], (byte)f[2], (byte)f[3]);
                else if (t == typeof(Rect) && f.Length == 4)
                    value = new Rect(f[0], f[1], f[2], f[3]);
                else if (t == typeof(Bounds) && f.Length == 6)
                    value = new Bounds(new Vector3(f[0], f[1], f[2]), new Vector3(f[3], f[4], f[5]));
                else
                    throw new FormatException("Unexpected component count.");
                error = null;
                return true;
            }
            catch (Exception ex)
        ...
        private static string Join(params object[] values) => string.Join(", ", Array.ConvertAll(values, value => Convert.ToString(value, CultureInfo.InvariantCulture)));
    }
    ...
    internal sealed class RuntimeObjectReferenceDrawer : IRuntimeValueDrawer
    {
        ...
        public string Format(object value, Type type) => value is Object obj && obj != null ? $"{obj.name} ({obj.GetInstanceID()})" : "null";
