...
namespace SAS.Utilities.RuntimeSceneInspector.Core
{
    ...
    internal abstract class RuntimeComponentDrawer<TComponent> : IRuntimeEditableComponentDrawer where TComponent : Component
    {
        ...
        private readonly ConditionalWeakTable<TComponent, InspectionCache> _inspectionCache = new();
        ...
        public IReadOnlyList<RuntimeMemberDescriptor> BuildInspector(Component component)
        {
            ...
            InspectionCache cache = _inspectionCache.GetValue(target, _ => new InspectionCache(_members.Count));
            bool changed = cache.Descriptors == null;
            for (int i = 0; i < _members.Count; i++)
            {
                MemberDefinition member = _members[i];
                bool applies = member.AppliesTo(target);
                if (cache.Applies[i] != applies)
                    changed = true;
                cache.Applies[i] = applies;
                if (!applies)
                {
                ...
                if (member.Capture(target, _valueDrawers, cache.Values[i], cache.HasValues[i], out object value))
                    changed = true;
            ...
            var descriptors = new List<RuntimeMemberDescriptor>(_members.Count);
            for (int i = 0; i < _members.Count; i++)
            {
                if (cache.Applies[i])
                    descriptors.Add(_members[i].BuildCaptured(cache.Values[i], _valueDrawers));
            }
        ...
        public bool TrySetValue(Component component, string memberId, string text, out RuntimeCommandResult result)
        {
            ...
            {
                if (!string.Equals(member.Id, memberId, StringComparison.Ordinal))
                    continue;
                ownsId = true;
            ...
            result = ownsId ? RuntimeCommandResult.Fail(component == null ? "The component no longer exists." : "This member is unavailable on the component subtype.") : null;
            return ownsId;
        }
        ...
        protected static string RequirePositive(float value, string displayName) => value > 0f ? null : displayName + " must be greater than zero.";

        protected static string RequireNonNegative(float value, string displayName) => value >= 0f ? null : displayName + " cannot be negative.";

        protected static string RequireNonNegative(Vector2 value, string displayName) => value.x >= 0f && value.y >= 0f ? null : displayName + " components cannot be negative.";

        protected static string RequireNonNegative(Vector3 value, string displayName) => value.x >= 0f && value.y >= 0f && value.z >= 0f ? null : displayName + " components cannot be negative.";
        ...
        private static bool IsNamedEnumValue(string text, Type enumType)
        {
            ...
            foreach (string supplied in suppliedNames)
            {
                ...
                {
                    if (!string.Equals(candidate, declared, StringComparison.OrdinalIgnoreCase))
                        continue;
                    found = true;
                    break;
                }
        ...
        private static string ValidateFinite(object value)
        {
            ...
            if (value is double d && (double.IsNaN(d) || double.IsInfinity(d)))
                return "Value must be finite.";
            if (value is Vector2 v2 && (!Finite(v2.x) || !Finite(v2.y)))
                return "Value must be finite.";
            ...
            if (value is Rect r && (!Finite(r.x) || !Finite(r.y) || !Finite(r.width) || !Finite(r.height)))
                return "Value must be finite.";
            if (value is Bounds b && (!Finite(b.center.x) || !Finite(b.center.y) || !Finite(b.center.z) || !Finite(b.size.x) || !Finite(b.size.y) || !Finite(b.size.z)))
                return "Value must be finite.";
            return null;
        }
        ...
        private abstract class MemberDefinition
        {
            ...
            public abstract bool Capture(TComponent component, RuntimeValueDrawerRegistry valueDrawers,
                object previousValue, bool hasPreviousValue, out object capturedValue);
            public abstract RuntimeMemberDescriptor BuildCaptured(object capturedValue, RuntimeValueDrawerRegistry valueDrawers);
            public abstract RuntimeCommandResult Set(TComponent component, string text, RuntimeValueDrawerRegistry valueDrawers);
        }
        ...
        private sealed class MemberDefinition<TValue> : MemberDefinition
        {
            ...
            public override bool Capture(TComponent component, RuntimeValueDrawerRegistry valueDrawers,
                object previousValue, bool hasPreviousValue, out object capturedValue)
            {
                try
                {
                    ...
                    {
                        Object unityObject = (object)value as Object;
                    ...
                    if (drawer == null)
                    {
                        ...
                        if (hasPreviousValue && previousValue is CapturedFormattedValue previousFormatted &&
                            string.Equals(previousFormatted.Value, formatted, StringComparison.Ordinal))
                        {
                            capturedValue = previousValue;
                            return false;
                        }

                        capturedValue = new CapturedFormattedValue(formatted);
                        return true;
                    }

                    if (hasPreviousValue && previousValue is TValue previousTyped &&
                        EqualityComparer<TValue>.Default.Equals(previousTyped, value))
                    {
                        capturedValue = previousValue;
                        return false;
                    }
            ...
            public override RuntimeMemberDescriptor BuildCaptured(object capturedValue, RuntimeValueDrawerRegistry valueDrawers)
            {
                ...
                else
                {
                    ...
                    object value = capturedValue is CapturedUnityObject unityObject
                        ? unityObject.Value
                        : capturedValue is CapturedFormattedValue formatted
                            ? formatted.Value
                            : capturedValue;
                    descriptor.Value = capturedValue is CapturedFormattedValue
                        ? (string)value
                        : drawer?.Format(value, ValueType) ?? value?.ToString() ?? "null";
                    descriptor.ReadOnly = _setter == null || drawer == null || typeof(Object).IsAssignableFrom(ValueType);
                }
            ...
            public override RuntimeCommandResult Set(TComponent component, string text, RuntimeValueDrawerRegistry valueDrawers)
            {
                ...
                if (!(parsed is TValue value))
                    return RuntimeCommandResult.Fail("Parsed value has the wrong type.");

                string finiteError = ValidateFinite(value);
                if (finiteError != null)
                    return RuntimeCommandResult.Fail(finiteError);
                string validationError = _validator?.Invoke(component, value);
                if (!string.IsNullOrEmpty(validationError))
                    return RuntimeCommandResult.Fail(validationError);
                ...
                try
                {
                    ...
                    if (hasPreviousValue && EqualityComparer<TValue>.Default.Equals(appliedValue, previousValue))
                    {
                        return RuntimeCommandResult.Fail("Unity rejected the requested value " + requestedText + "; the value remains " + appliedText + ".");
                    }

                    return RuntimeCommandResult.Ok("Unity applied " + appliedText + " instead of the requested " + requestedText + ".");
                }
                catch (Exception ex)
                {
                    return RuntimeCommandResult.Fail("Unity accepted the setter call, but the applied value could " + "not be verified (" + ex.GetType().Name + ": " + ex.Message + ").");
                }
            }
        ...
        {
            public InspectionCache(int count)
            {
                Applies = new bool[count];
            ...
            public IReadOnlyList<RuntimeMemberDescriptor> Descriptors { get; set; }
        }

        private sealed class CapturedUnityObject
        {
            public CapturedUnityObject(Object value, int instanceId, string name)
            {
                Value = value;
        ...
        private sealed class CapturedFormattedValue
        {
            public CapturedFormattedValue(string value) => Value = value;
            public string Value { get; }
        }

        private sealed class CapturedError
        {
            public CapturedError(string value) => Value = value;
            public string Value { get; }
        }
    }
}
