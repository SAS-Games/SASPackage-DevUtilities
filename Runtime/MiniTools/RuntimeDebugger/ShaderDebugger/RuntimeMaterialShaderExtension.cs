...
namespace SAS.Utilities.RuntimeDebugger.Core
{
    ...
    /// </summary>
    internal sealed class RuntimeMaterialShaderExtension : IRuntimeDebuggerInspectorExtension
    {
        ...
        internal RuntimeMaterialShaderExtension(RuntimeDebuggerSettings settings)
        {
        ...
        public bool TryExecute(RuntimeDebuggerCommand command, RuntimeObjectRegistry registry,
        ...
        private RuntimeMaterialSlotDescriptor BuildSlot(Renderer renderer, int materialIndex, Material material)
        {
            ...
            slot.IsDebuggerMaterialInstance = IsDebuggerInstance(renderer, materialIndex, material);
            if (shader == null)
                return slot;
            ...
            foreach (RuntimeShaderPropertyDescriptor property in shaderDescriptor.Properties)
            {
                ...
                try
                {
                    ...
                    bool debuggerOverride = _propertyBlockOverrides.TryGetValue(key, out PropertyBlockOverrideRecord record);
                    if (debuggerOverride)
                    {
                        value = record.CurrentValue;
                        source = "Debugger renderer override";
                    }
                    ...
                    properties.Add(new RuntimeShaderPropertyView
                    {
                        ...
                        Value = Format(value),
                        ValueSource = source,
                        HasDebuggerOverride = debuggerOverride,
            ...
            switch (command.Scope)
            {
                case RuntimeMaterialEditScope.RendererPropertyBlock:
                {
                    ...
                    if (!_propertyBlockOverrides.TryGetValue(key, out PropertyBlockOverrideRecord record))
                        return RuntimeCommandResult.Fail("The debugger has no renderer override for this property.");
                ...
                case RuntimeMaterialEditScope.MaterialInstance:
                {
                    ...
                    if (!_materialInstances.TryGetValue(key, out MaterialInstanceRecord instance) ||
                        ...
                        return RuntimeCommandResult.Fail("The debugger has no material instance for this slot.");
            ...
            switch (command.Scope)
            {
                case RuntimeMaterialEditScope.RendererPropertyBlock:
                {
                    ...
                    return restored > 0
                        ...
                        : RuntimeCommandResult.Fail("The debugger has no renderer overrides for this slot.");
                }
                case RuntimeMaterialEditScope.MaterialInstance:
                    return RestoreMaterialInstance(renderer, command.MaterialIndex)
                        ? RuntimeCommandResult.Ok("Original material restored.")
                        : RuntimeCommandResult.Fail("The debugger has no material instance for this slot.");
                case RuntimeMaterialEditScope.SharedMaterial:
                {
                    if (!TryGetScopeMaterial(renderer, command.MaterialIndex, command.Scope, out Material material,
                            out error))
                    ...
                    return restored > 0
                        ...
                        : RuntimeCommandResult.Fail("The debugger has no shared-material changes for this slot.");
                }
                case RuntimeMaterialEditScope.GlobalShaderProperty:
                {
                    if (!TryGetScopeMaterial(renderer, command.MaterialIndex, command.Scope, out Material material,
                    ...
                    return restored > 0
                        ...
                        : RuntimeCommandResult.Fail("The debugger has no global changes for this shader.");
                }
                default:
                    return RuntimeCommandResult.Fail("Unsupported material edit scope.");
            }
            ...
            RuntimeShaderPropertyDescriptor property, RuntimeShaderPropertyValue value)
        {
            ...
            return RuntimeCommandResult.Ok("Applied to a debugger-owned material instance.");
        }
            ...
            out string error)
        {
            ...
            if (_materialInstances.Count >= _settings.MaxDebuggerMaterialInstances)
            {
                instance = null;
                error = "The debugger material-instance limit has been reached.";
                return false;
            }
            ...
            {
                name = original.name + " [Runtime Debugger]",
        ...
        private bool IsDebuggerInstance(Renderer renderer, int materialIndex, Material material)
        {
            var key = new SlotKey(renderer.GetInstanceID(), materialIndex);
            ...
            RuntimeShaderPropertyDescriptor property)
        {
            var key = new MaterialPropertyKey(material.GetInstanceID(), property.PropertyId);
            if (!_sharedMaterialOverrides.TryGetValue(key, out MaterialOverrideRecord record))
                return RuntimeCommandResult.Fail("The debugger has no shared-material change for this property.");
        ...
        {
            if (!_globalOverrides.TryGetValue(property.PropertyId, out GlobalOverrideRecord record))
                return RuntimeCommandResult.Fail("The debugger has no global change for this property.");
